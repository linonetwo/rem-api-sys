...
pub fn handle_function_prototype(
    ...
    match configs.method_flavor {
        ...
        MethodFlavor::ApiTrait => {
            ...
            if raw_camel_case_name == "GetApiVersion"
                || raw_camel_case_name == "CreateFtdcTraderApi"
            {
                // CThostFtdcTraderApi_GetApiVersion 和 CThostFtdcTraderApi_CreateFtdcTraderApi 是静态方法
                return lines;
            }
            ...
            //         unsafe {{
            //             ((*(*self).vtable_).CThostFtdcTraderApi_start)(self as *mut CThostFtdcTraderApi, p_listener as *mut YDListener)
            //         }}
            //     }}
            // "#,
            //                 ));
...
fn get_rs_result_type_from_c_result_type(c_result_type: &str) -> String {
    match c_result_type {
        ...
        "CThostFtdcTraderApi *" => "*mut CThostFtdcTraderApi".to_string(),
        _ => {
            if c_result_type.starts_with("const ") && c_result_type.ends_with(" *") {
                ...
                format!(
                    "/** {} (generated by get_rs_result_type_from_c_result_type) */",
                    c_result_type
                )
            }
        }
