...
#define SL_EES_API_VERSION    "4.2.4.72"				///<  api版本号
...
#define EES_SideType_today_opt_exec				11		///< =行权（平今）
#define EES_SideType_ovn_opt_exec				12		///< =行权（平昨）
#define EES_SideType_opt_exec					13		///行权（平仓）
#define EES_SideType_abandon_today_opt_exec		14		///< =放弃今仓行权
#define EES_SideType_abandon_ovn_opt_exec		15		///< =放弃昨仓行权
#define EES_SideType_abandon_opt_exec	  		16		///放弃行权（平仓）
#define EES_sideType_opt_offset					17		///期权自对冲
#define EES_sideType_perform_offset				18		///履约对冲
...
#define EES_ExchangeID_gfex						108     ////<=广期所
...
#define ELR_USER_IN_FUSING						9		///< 登录熔断中
#define ELR_MISSING_UDP_PORT					10		///< 登录时未指定UDP端口
...
{ 
	...
	EES_SideType        m_Side;							///< 买卖方向 11 =行权（平今）12=行权（平昨）14=放弃今仓行权15=放弃昨仓行权
	EES_ExchangeID      m_Exchange;						///< 交易所
	EES_Symbol          m_Symbol;						///< 合约代码
	EES_SecType         m_SecType;						///< 交易品种
	double              m_Price;						///< 价格
	unsigned int        m_Qty;							///< 数量
	EES_ForceCloseReason m_ForceCloseReason;			///<强平原因  0=非强平, 1=资金不足,2=客户超仓,3=会员超仓,4=持仓非整数倍,5=违规，6=其它
	...
	unsigned char		m_IsOffset;						///< 行权后是否对冲（1是，0否）
	unsigned short		m_BasketId;						///< 篮子号 0是普通单  1是预埋散单 1以上是预埋单
	EES_EnterOrderField()
	{
...
///下预埋单触发
struct EES_EnterOrigOrderField
{
	EES_MarketToken     m_MarketOrderToken;				///< 散单需要用单号一一触发撤单。用篮子号触发时，填0
	unsigned short		m_BasketId;						///< 篮子号 0是普通单  1是预埋散单 1以上是预埋单
	EES_Account         m_Account;						///< 用户账户
	unsigned int        m_OrigUserId;                   ///< 触发的User_id,支持触发同个账户下，不同用户的userId
	EES_EnterOrigOrderField()
	{
		memset(this, 0, sizeof(*this));
	}
};
...
///预埋单回报
struct EES_OrigOrderReportField
{
	EES_MarketToken     m_MarketOrderToken;				///< 市场里面挂单的token
	EES_UserID          m_UserID;						///< 订单的 user id 
	EES_Account         m_Account;						///< 用户代码
	unsigned short		m_BasketId;						///< 篮子号 0是普通单  1是预埋散单 1以上是预埋单
	EES_Nanosecond      m_AcceptTime;					///< 从1970年1月1日0时0分0秒开始的纳秒时间，请使用ConvertFromTimestamp接口转换为可读的时间
	unsigned int		m_BasketOrderNum;				///< 触发预埋的篮子内，预埋单的总量
	unsigned int		m_ReasonCode;					///< 触发结果 如果是0 则就没问题
	EES_ReasonText		m_ReasonText;					///< 触发结果错误中文原因
};
...
struct EES_CancelOrder
{
	...
	unsigned char   m_orderIdType;                      ///< 0 系统单号撤单  1 本地单号撤单（需要填写本地单号m_ClientOrderToken） 2 撤销非散单篮子
	EES_ClientToken  m_ClientOrderToken;				///< 原来本地单号
	unsigned int    m_origOrderUserId;                  ///< 可以为指定的user_Id 也可以为0 0会把登录的user_id自动补充(在orderIdType是1和2的情况下可以填写)
	unsigned short	m_BasketId;							///< 篮子号 0是普通单  1是预埋散单 1以上是预埋单 
	EES_CancelOrder()
	{
		memset(this, 0, sizeof(*this));
...
struct EES_QueryAccountOrder
{
	...
	EES_HedgeFlag		m_HedgeFlag;					///< 投机套利标志
	EES_OrderStatus		m_BasketStatus;					///< 0=未触发，1=已触发 2=已关闭
	unsigned short		m_BasketId;						///< 篮子号 0是普通单  1是预埋散单 1以上是预埋单
};
...
struct EES_BasketState
{
	EES_Account			m_Account;						///< 帐户ID 
	EES_UserID			m_UserID;						///< 账户下对应的userId
	EES_OrderStatus		m_BasketStatus;					///< 0=未触发，1=已触发 2=已关闭
	unsigned short		m_BasketId;						///< 篮子号 0是普通单  1是预埋散单 1以上是预埋单
	EES_Nanosecond		m_TriggerTime;					///< 篮子触发时间，从1970年1月1日0时0分0秒开始的纳秒时间，请使用ConvertFromTimestamp接口转换为可读的时间
	unsigned int        m_BasketOrderNum;				///< 篮子中的单子数量	
	EES_MarketToken		m_MarketOrderToken;				///< 盛立系统产生的单子号，和盛立交流时可用该号。

};

struct EES_BasketStateFinish
{
	EES_Account			m_Account;						///< 帐户ID 
	EES_UserID			m_UserID;						///< 账户下对应的userId
	unsigned short		m_BasketId;						///< 篮子号 0是普通单  1是预埋散单 1以上是预埋单
...
struct EES_AccountCashInout
{
	EES_Account			m_account;						///< Value  Notes
	double				m_AvailableBp;					///< 总可用资金
	double				m_CashInOut;	///< 	本次出入金
	double				m_CashInTotal; ///< 	入金累计
	double				m_CashOutTotal; ///< 	出金累计
...
struct EES_TradeSvrInfo
{
	...
	bool			m_TimestampSwitch;	     /// 是否开启硬件时间戳测试功能
	bool			m_BlockRecvSwitch;	     /// 是否开启阻塞接收数据
	bool            m_EfviEnterSwitch;       /// 报单是否用efvi方式，默认1，0：普通模式
	bool            m_HandleSignalSwitch;    /// 是否需要处理Ctrl+C信号 
	short			m_ReqCpuId;			     /// 上行绑核 填-1，不绑定
	short			m_RspCpuId;			     /// 下行绑核 填-1，不绑定
...
{
	char				m_TradeNo[17];///交易席位号
	unsigned int		m_Result;///0:成功，1:用户绑定的账户没有做市类型，无权限，2:当前系统绑定不存在该交易所席位，或者请求的交易所和席位对应错误
	int			        m_SessionID;///交易所席位代号
};
